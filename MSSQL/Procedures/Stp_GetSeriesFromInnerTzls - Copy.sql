...
/****** Object:  StoredProcedure [dbo].[Stp_GetSeriesFromInnerTzls]    Script Date: 22/04/2026 09:29:44 ******/
SET ANSI_NULLS ON
GO
...
--with @convert tbc - I have freqtable tables FreqDates_ANN ' FreqDates_MON ' if @convert is not OFF
 set @sql =
   'declare @mnull varchar(10) = '+ quotename('null',char(39)) +
    + @grpfor
  + ' select top('+cast(@top as nvarchar(32)) + ')'
  + ' f.FreqDate as DynDate,'
  + ' cast(cast(f.FreqDate as datetimeoffset) AT TIME ZONE '+@targettimezoneq+' as datetime) as VDate,'
  + ' (c.Value)*'+@multiplier+'/'+@basis_txt+' as Value, '
  + quotename('[',char(39))
  + '+ convert(varchar,'
  + ' convert(bigint, datediff_big(MILLISECOND, '+ quotename('01-01-1970 00:00:00',char(39)) +', cast(cast(f.FreqDate as
  datetimeoffset) AT TIME ZONE '+@targettimezoneq+' as datetime))) *1) +'
  + quotename(', ',char(39))
  + ' + isnull(convert(varchar,convert(decimal(38,8),(c.Value)*'+@multiplier+'/'+@basis_txt+')), @mnull) +'
  + quotename(']',char(39))
  + ' as Epodateval '
  + ', '+@curveidstr + ' as CurveId'
  + ' from FreqDates_ANN f'
  + ' left join '+ @tablename + ' c on c.VDate = f.FreqDate'
  + '   and c.CurveId = ' + cast(@mycurveid as nvarchar(16))
  + ' where f.FreqDate >= cast(' + quotename(cast(@mystart as varchar(32)),char(39)) + ' as datetime)'
